...
export type CandidateDocument={id:string;document_type:string;file_name:string;storage_path:string;mime_type:string|null;file_size:number|null;verification_status:"pending"|"verified"|"rejected";verification_note:string|null;created_at:string;updated_at:string};
...
const documentSelection="id,document_type,file_name,storage_path,mime_type,file_size,verification_status,verification_note,created_at,updated_at";
...
export async function uploadCandidateDocument(documentType:string,file:File):Promise<CandidateDocument>{const user=await requireCandidateUser();if(file.size>10*1024*1024)throw new Error("FILE_TOO_LARGE");if(!["application/pdf","image/jpeg","image/png"].includes(file.type))throw new Error("INVALID_FILE_TYPE");const safeName=file.name.replace(/[^a-zA-Z0-9._-]/g,"-");const storagePath=`${user.id}/${documentType}/${crypto.randomUUID()}-${safeName}`;const uploaded=await supabase.storage.from("candidate-documents").upload(storagePath,file,{upsert:false});if(uploaded.error)throw uploaded.error;const inserted=await supabase.from("candidate_documents").insert({candidate_id:user.id,document_type:documentType,file_name:file.name,storage_path:storagePath,mime_type:file.type||null,file_size:file.size,verification_status:"pending"}).select(documentSelection).single();if(inserted.error){await supabase.storage.from("candidate-documents").remove([storagePath]);throw inserted.error}return inserted.data as CandidateDocument}
